defmodule ExFix.Session do
  ...
  def session_start(%Session{config: config, out_lastseq: lastseq} = session) do
    ...
    fields = [
      {@field_logon_encrypt_method, logon_encrypt_method},
  ...
  def send_message(
        %Session{config: config, out_lastseq: out_lastseq, out_queue: out_queue} = session,
        out_message
      ) do
    out_lastseq = out_lastseq + 1
  ...
  def process_incoming_message(
        expected_seqnum,
        @msg_type_logon = msg_type,
        session_name,
        %Session{config: config} = session,
        %InMessage{seqnum: _seqnum} = msg
      ) do
    ...
    {:ok, [],
     %Session{session | in_lastseq: expected_seqnum, status: :online, extra_bytes: msg.other_msgs}}
  end

  def process_incoming_message(
        _expected_seqnum,
        @msg_type_test_request = msg_type,
        session_name,
        %Session{config: config, out_lastseq: out_lastseq} = session,
        %InMessage{seqnum: seqnum} = msg
      ) do
    %SessionConfig{session_handler: session_handler, env: env} = config
    session_handler.on_session_message(session_name, msg_type, msg, env)
    out_lastseq = out_lastseq + 1
    ...
    {:ok, [hb_msg],
     %Session{session | in_lastseq: seqnum, out_lastseq: out_lastseq, extra_bytes: msg.other_msgs}}
  end

  def process_incoming_message(
        expected_seqnum,
        @msg_type_resend_request = msg_type,
        session_name,
        %Session{config: config, out_queue: out_queue} = session,
        %InMessage{seqnum: _seqnum, fields: fields} = msg
      ) do
    %SessionConfig{session_handler: session_handler, env: env} = config
    session_handler.on_session_message(session_name, msg_type, msg, env)
  ...
  def process_incoming_message(
        expected_seqnum,
        @msg_type_sequence_reset = msg_type,
        session_name,
        %Session{config: config} = session,
        %InMessage{seqnum: seqnum, fields: fields} = msg
      ) do
    %SessionConfig{session_handler: session_handler, env: env} = config
    session_handler.on_session_message(session_name, msg_type, msg, env)
  ...
  def process_incoming_message(
    ...
    {:ok, [logout_msg],
     %Session{session | in_lastseq: seqnum, status: :disconnecting, extra_bytes: msg.other_msgs}}
  end
  ...
  def process_incoming_message(
    ...
    {:ok, [],
     %Session{session | in_lastseq: seqnum, status: :offline, extra_bytes: msg.other_msgs}}
  end

  def process_incoming_message(
        _expected_seqnum,
        @msg_type_heartbeat = msg_type,
        session_name,
        %Session{config: config} = session,
        %InMessage{seqnum: seqnum} = msg
      ) do
  ...
  def process_incoming_message(
        ...
        session_name,
        %Session{config: config, out_lastseq: out_lastseq} = session,
        %InMessage{poss_dup: false, fields: fields} = msg
      ) do
    %SessionConfig{
    ...
    case :lists.keyfind(@field_orig_sending_time, 1, fields) do
      ...
      _ ->
        ...
        reject_msg =
          build_message(config, @msg_type_reject, out_lastseq, [
            {@field_session_reject_reason, "1"},
            {@field_text, "Required tag missing: OrigSendingTime"}
          ])
  ...
  def handle_timeout(
        %Session{config: config, out_lastseq: out_lastseq, last_test_req_id_sent: nil} = session,
        :rx
      ) do
    out_lastseq = out_lastseq + 1
    test_req_id = generate_id()
    test_msg = build_message(config, @msg_type_test_request, out_lastseq, [{"112", test_req_id}])

    {:ok, [test_msg],
     %Session{session | out_lastseq: out_lastseq, last_test_req_id_sent: test_req_id}}
  end
  ...
  def process_invalid_message(
        %Session{config: config, out_lastseq: out_lastseq} = session,
        _expected_seqnum,
        %InMessage{valid: false, error_reason: :begin_string_error} = msg
      ) do
    out_lastseq = out_lastseq + 1
  ...
  def process_invalid_message(
    ...
    resend_request =
      build_message(config, @msg_type_resend_request, out_lastseq, [
        {@field_begin_seq_no, init_gap},
        {@field_end_seq_no, end_gap}
      ])

    session = add_to_in_queue(session, msg)

    {:ok, [resend_request],
     %Session{session | out_lastseq: out_lastseq, extra_bytes: msg.other_msgs}}
  end
  ...
  defp build_message(config, msg_type, seqnum, body) do
    %SessionConfig{sender_comp_id: sender, target_comp_id: target, time_service: time_service} =
      config
  ...
  defp add_to_in_queue(
         %Session{in_queue: in_queue} = session,
         %InMessage{valid: false, error_reason: :unexpected_seqnum} = msg
       ) do
  ...
  defp process_sequence_reset(
    ...
    reject_msg =
      build_message(config, @msg_type_reject, out_lastseq, [
        {@field_text, "Attempt to lower sequence number, invalid value NewSeqNum=#{new_seq_no}"}
      ])
