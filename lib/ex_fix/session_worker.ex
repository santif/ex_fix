defmodule ExFix.SessionWorker do
  ...
  def handle_call(
        :stop,
        _from,
        %State{transport: transport, client: client, session: session} = state
      ) do
    state =
      case Session.session_stop(session) do
  ...
  defp connect_and_send_logon(config, %State{name: fix_session_name} = state) do
    ...
    case config.transport_mod.connect(str_host, port, options) do
      {:ok, client} ->
        ...
        state = %State{
          state
          | transport: config.transport_mod,
            client: client,
            session: session,
            tx_timer: tx_timer
        }
